  ...
  export class AppComponent implements OnInit {
    ...
    mensajeWarmup   = 'Iniciando servidor...';
      ...
      private readonly rutasSinNavbar = ['/', '/completar-perfil', '/suscripcion','/suscripcion/confirmado','/callback' ];
      mostrarNavbar = false;
    ...
    this.mostrarNavbar = !this.rutasSinNavbar.includes(window.location.pathname);
      ...
      if (!loading) {
        this.auth.isAuthenticated$.subscribe(isAuth => {
          ...
          if (isAuth && (rutaActual === '/' || rutaActual === '/callback')) {
    ...
    private redirigirSegunPerfil() {
      this.usuarioService.getPerfil().subscribe({
        next: (user: any) => {
          const esAdminSistema = user.rol === 'ADMIN';

          if (esAdminSistema) {
            this.router.navigate(['/admin']);
          } else if (user.tipoUsuario === 'EMPRESA') {
            this.router.navigate(['/company']);
          }else if (user.tipoUsuario === 'HOGAR') {
          // Si eres un usuario residencial registrado, directo a ver tus sensores
          this.router.navigate(['/mis-sensores']);
          } else {
